...
public class App extends Application {
  ...
  public void start(Stage stage) {
    ...
    scene.setOnKeyPressed(evt -> {
      switch (evt.getCode()) {
        case C:  // Note: this is not capital C
          removeLastShape(); // Removes shapecount
          break;
      }
    });
  ...
  private HBox makeToolPanel(Canvas canvas) {
    ...
    tools.getChildren().add(counterLabel);
  ...
  private void addShape(Shape shape) {
    ...
    shapeCount++;
    paintCanvas();
    counterLabel.setText("Shapes: " + shapeCount); 
  }
  ...
  private void removeLastShape() {
    if (shapeCount > 0) {
      shapeCount--;          // Just decrement the count
      paintCanvas();         // Redraw the canvas
      counterLabel.setText("Shapes: " + shapeCount);
    }
  }
